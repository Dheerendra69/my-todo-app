...
const statusApiValues: Record<Status, TaskStatus> = {
  "To Do": "todo",
  Doing: "doing",
  Completed: "completed",
  "On Hold": "on_hold",
...
const statusLabels: Record<TaskStatus, Status> = {
  todo: "To Do",
  doing: "Doing",
  completed: "Completed",
  on_hold: "On Hold",
...
export default function AddTaskModal({
  ...
  useEffect(() => {
    ...
    setStatus(statusLabels[defaultStatus]);
    ...
    if (!projectId) {
      setError("Please create a project before creating a task.");
    }

    fetchMembers();
  }, [isOpen, defaultStatus, isGuest, error]);
  ...
  const toggleMember = (memberToToggle: Member) => {
    setSelectedMembers((current) => {
      ...
      if (alreadySelected) {
        return current.filter((member) => member.id !== memberToToggle.id);
      }

      return [...current, memberToToggle];
    });
  };
  ...
  const addLabel = () => {
    ...
    const formattedLabel = toTitleCase(trimmedLabel);

    const alreadyExists = selectedLabels.some(
      (label) => label.name.toLowerCase() === formattedLabel.toLowerCase(),
    );
  ...
  const removeLabel = (labelName: string) => {
    setSelectedLabels((current) =>
      current.filter((label) => label.name !== labelName),
    );
  };
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tasks`, {
        ...
        body: JSON.stringify({
          ...
          priority: priorityApiValues[priority],
          status: statusApiValues[status],
      ...
      const data: BackendCreatedTask = await response.json();
      ...
      const createdMembers = data.members ?? selectedMembers;
      ...
      onCreate({
        ...
        assignee: data.assignee
          ? {
            ...
            avatar: data.assignee.avatar ?? null,
          }
          : null,
        ...
        labels: data.labels ?? selectedLabels,
      });
      ...
      >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <button
                  type="button"
                  onClick={() => {
                    setPriorityOpen((current) => !current);
                ...
                >
                  <span className={priorityStyles[priority].text}>
                    {priorityIcon}
                  </span>
                ...
                {priorityOpen && (
                  <div className="absolute left-0 top-10.5 z-20 w-full rounded-md border border-border bg-background p-1 shadow-[0_4px_12px_rgba(0,0,0,0.12)]">
                    {priorities.map((item) => (
                      <button
                        key={item}
                        type="button"
                      ...
                      >
                        <span className={priorityStyles[item].text}>
                          {item === "Low" || item === "No Priority" ? (
                            <SignalLow size={14} />
                          ) : (
                            <SignalHigh size={14} />
                          )}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="h-9 w-full rounded-md border border-border bg-background px-3 pl-9 text-sm text-foreground"
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {selectedMembers.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {selectedMembers.map((member) => (
                  <div
                    key={member.id}
                    className="flex h-7 items-center gap-1.5 rounded-full border border-border bg-surface-secondary py-1 pl-1 pr-2 text-xs text-foreground"
                  >
                    {member.avatar ? (
                      <img
                        src={member.avatar}
                      ...
                      <div className="flex h-5 w-5 items-center justify-center rounded-full bg-foreground text-[9px] font-medium text-background">
                        {getInitials(member.name)}
                      </div>
                    )}

                    <span className="max-w-30 truncate">{member.name}</span>

                    <button
                      type="button"
                      onClick={() => removeMember(member.id)}
                      className="flex h-4 w-4 items-center justify-center rounded-full hover:bg-border"
                    >
                      <X size={11} />
                    </button>
                  </div>
            ...
            <div className="relative">
              <button
                ...
                disabled={isGuest}
                className="flex h-9 w-full items-center gap-2 rounded-md border border-border bg-background px-3 text-left disabled:cursor-not-allowed disabled:opacity-60"
              >
                <UserPlus size={14} />
                ...
                {!isGuest && <ChevronDown size={14} />}
              </button>

              {!isGuest && memberOpen && (
                <div className="absolute left-0 top-10.5 z-30 w-full rounded-md border border-border bg-background p-2 shadow-[0_4px_12px_rgba(0,0,0,0.12)]">
                  <div className="relative mb-2">
                    ...
                    <input
                      ...
                      className="h-9 w-full rounded-md border border-border bg-background pl-8 pr-3 text-sm text-foreground outline-none placeholder:text-foreground-secondary focus:border-foreground-secondary"
                    />
                  </div>

                  <div className="max-h-55 overflow-y-auto">
                    {membersLoading && (
                      <div className="flex h-16 items-center justify-center text-xs text-foreground-secondary">
                        Loading members...
                      </div>
                    )}
                      ...
                      filteredMembers.map((member) => {
                        ...
                        return (
                          <button
                            ...
                            className="flex min-h-10 w-full items-center gap-2 rounded-md px-2 py-1.5 hover:bg-surface-secondary"
                          >
                            {member.avatar ? (
                              <img
                                src={member.avatar}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <button
                ...
                className="flex h-9 w-full items-center gap-2 rounded-md border border-border bg-background px-3 text-left"
              >
                <span className="flex-1 text-sm text-foreground">{status}</span>
                ...
                <div className="absolute left-0 top-10.5 z-20 w-full rounded-md border border-border bg-background p-1 shadow-lg">
                  {statuses.map((item) => (
                    <button
                      ...
                      className="flex h-9 w-full items-center rounded-md px-2 hover:bg-surface-secondary"
                    >
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border p-4">
          <button
            ...
            onClick={onClose}
            disabled={isSubmitting}
            className="h-9 rounded-md px-3 text-sm font-medium text-foreground hover:bg-surface-secondary"
          >
            Cancel
          </button>
